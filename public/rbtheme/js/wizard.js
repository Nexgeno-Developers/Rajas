...
function countdown() {
  ...
  if (!stopTimer) setTimeout("countdown()", 1000);
}
...
(function ($) {
  ...
  var wizardInit = function wizardInit() {
    ...
    wizards.forEach(function (wizard) {
      ...
      var selectCategory = wizard.querySelector(
        "[data-wizard-validate-category]"
      );
      var selectService = wizard.querySelector(
        "[data-wizard-validate-service]"
      );
      var selectEmployee = wizard.querySelector(
        "[data-wizard-validate-employee]"
      );
      ...
      var inputFirstName = wizard.querySelector(
        "[data-wizard-validate-first-name]"
      );
      var inputLastName = wizard.querySelector(
        "[data-wizard-validate-last-name]"
      );
      ...
      var allowedPerson = wizard.querySelector(
        "[data-wizard-validate-allowed-person]"
      ); //new
      var allowedWeight = wizard.querySelector(
        "[data-wizard-validate-allowed-weight]"
      ); //new
      var govermentId = wizard.querySelector(
        "[data-wizard-validate-goverment-id]"
      ); //new
      ...
      nextButton.addEventListener("click", function () {
        if (count == 0) {
          if (
            !selectService.options[selectService.options.selectedIndex].value ||
            (selectEmployee.nodeName == "INPUT"
              ? !selectEmployee.value
              : !selectEmployee.options[selectEmployee.options.selectedIndex]
                  .value) ||
            !selectDate.value
          ) {
            ...
            if (custom == 1) {
              if (
                selectCategory.options[selectCategory.options.selectedIndex]
                  .value != ""
              ) {
                $(".category_name").html(
                  selectCategory.options[selectCategory.options.selectedIndex]
            ...
            $(".service_name").html(
              selectService.options[selectService.options.selectedIndex].text
            );
            ...
            $(".booking_date").html(
              moment(selectDate.value).format("MMMM DD, YYYY")
            );
            $(".booking_price").html(
              selectService.options[
                selectService.options.selectedIndex
            ...
            $("#bootstrap-wizard-allowed-person").attr(
              "max",
              selectService.options[
                selectService.options.selectedIndex
              ].getAttribute("data-allowed-person")
            );
            $("#bootstrap-wizard-allowed-weight").attr(
              "max",
              selectService.options[
                selectService.options.selectedIndex
              ].getAttribute("data-allowed-weight")
            );
            allowedPerson.nextElementSibling.innerHTML =
              "Maximum number of persons allowed is " +
              allowedPerson.getAttribute("max");
            allowedWeight.nextElementSibling.innerHTML =
              "Maximum weight allowed is " +
              allowedWeight.getAttribute("max") +
              " kg";
            //new
          ...
          if (!selectSlot.value) {
            ...
            if (document.querySelector("#msg").innerHTML == "") {
              document.querySelector("#msg").innerHTML =
                translate.please_select_appointment_slot;
          ...
          if (
            ...
            if (!inputEmail.value) {
              document.querySelector(".email-error").innerHTML =
                translate.please_enter_the_email;
            }
            if (!inputPhone.value) {
              document.querySelector(".phone-error").style.display = "block";
            }
            ...
            inputFirstName.value &&
            !validatePattern(/^([a-zA-z])+$/, inputFirstName.value)
          ) {
            inputFirstName.nextElementSibling.innerHTML =
              translate.please_enter_only_characters;
            inputFirstName.nextElementSibling.style.display = "block";
          } else if (
            inputLastName.value &&
            !validatePattern(/^([a-zA-z])+$/, inputLastName.value)
          ) {
            inputLastName.nextElementSibling.innerHTML =
              translate.please_enter_only_characters;
            ...
            inputPhone.value &&
            !validatePattern(phonePattern, inputPhone.value)
          ) {
            // form.classList.add('was-validated');
            document.querySelector(".phone-error").style.display = "block";
            document.querySelector(".phone-error").innerHTML =
              translate.phone_should_be_digits;
          } else if (
            parseFloat(allowedPerson.value) == 0 ||
            parseFloat(allowedPerson.value) >
              parseFloat(allowedPerson.getAttribute("max"))
          ) {
            form.classList.add("was-validated");
            allowedPerson.nextElementSibling.innerHTML =
              "Maximum number of persons allowed is " +
            ...
            parseFloat(allowedWeight.value) == 0 ||
            parseFloat(allowedWeight.value) >
              parseFloat(allowedWeight.getAttribute("max"))
          ) {
            form.classList.add("was-validated");
            allowedWeight.nextElementSibling.innerHTML =
              "Maximum weight allowed is " +
      ...
      if (tabToggleButtonEl.length) {
        tabToggleButtonEl.forEach(function (item, index) {
          ...
          item.addEventListener("show.bs.tab", function (e) {
            ...
            if (step == 0) {
              if (
                !selectService.options[selectService.options.selectedIndex]
                  .value ||
                (selectEmployee.nodeName == "INPUT"
                  ? !selectEmployee.value
                  : !selectEmployee.options[
                      selectEmployee.options.selectedIndex
                    ].value) ||
                !selectDate.value
              ) {
                e.preventDefault();
                ...
                var service_id = $(
                  "#bootstrap-wizard-service option:selected"
                ).data("id");
                if (selectEmployee.nodeName == "INPUT") {
                  var employee_id = selectEmployee.value;
                } else {
                  var employee_id = $(
                    "#bootstrap-wizard-employee option:selected"
                  ).val();
                }
                      ...
                      if (response.slots.length > 0) {
                        ...
                        $.each(response.slots, function (key, value) {
                          ...
                          if (response.book_time.length > 0) {
                            ...
                            var slot_end_time = new Date(
                              selectedDate + " " + value.end_time
                            );
                            ...
                            $.each(response.book_time, function (k, val) {
                              ...
                              var end_time = moment(
                                new Date(val.date + " " + val.finish_time)
                              ).format("hh:mm A");
                              ...
                              var google_start_time = new Date(
                                val.date + " " + start_time
                              );
                              var google_end_time = new Date(
                                val.date + " " + end_time
                              );
                          ...
                          html +=
                            '<button type="button" class="bookly-hour ' +
                            hide_slots +
                            '" data-value="' +
                            value.start_time +
                            "-" +
                            ...
                            ">";
                          html +=
                            '<span class="ladda-label bookly-time-main bookly-' +
                            weight +
                            '">';
                          html +=
                        ...
                        $("#msg").append(
                          '<p class="booked_msg" style="color:red;">' +
                            translate.booking_not_available +
                            "</p>"
                        );
                        $("#time-slots").html("");
                      }
